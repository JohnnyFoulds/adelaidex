...
/*
 * Draw a doughnut section
 */
void drawDoughnutSection(float x, float y, float innerRadius, float outerRadius, float startingAngle, float endAngle, color fill) {
  ...
  // draw individual lines to fill the section
...
void drawDoughnutChart(Table data, String column, float doughnutWidth, float dougnutHeight) {
  ...
  for (int rowIndex = 0; rowIndex < data.getRowCount(); rowIndex++) {
    ...
    drawDoughnutSection(chart_x, chart_y, (chartSize * 0.5) / 2, chartSize/2, lastAngle, lastAngle+radians(360 * dataRatio), ALT_CHART_COLORS[rowIndex]);
