...
float[][] loadData() {
  ...
  if (DEBUG) {
    float[][] testData = {
      ...
      { 7.4, 7.4, 7.5, 7.3, 7.6, 7.5, 7.5, 7.4, 7.3, 7.3, 7.2, 7.4, 7.5, 7.5, 7.5, 7.4, 7.2, 7.3, 7.5, 7.6, 7.2, 7.5, 7.6  }, 
...
void drawChart(float[][] data, float[][] dataSummary, float[][] overallSummmary, float chartWidth, float chartHeight) {
  ...
  float clusterSize = (chartWidth-20) / (data[0].length + 1);
  for (int i = 0; i < data[0].length; i++) {
    text(nfc(data[0][i], 0), i * clusterSize + leftOffset + clusterSize, chartHeight-3);
  }

  //draw the vertical labels
  float bottomValue = (overallSummmary[1][0]);
  float topValue = ceil(overallSummmary[1][1]);
  ...
  for (int i = 0; i < AXIS_INTEVALS+1; i++) {
    //text(nfc(bottomValue + (i * increment), 1), leftOffset-5, (chartHeight-10) - (i * axisHeight/AXIS_INTEVALS));
    text(nfc(bottomValue + (i * increment), 2), leftOffset-5, (chartHeight-10) - (i * axisHeight/AXIS_INTEVALS));
  }
  
  // get the max bar height
  //float maxBarHeight = (chartHeight-10) - (AXIS_INTEVALS * axisHeight/AXIS_INTEVALS);
  fill(CHART_COLORS[0]);
  rect(leftOffset, axisHeight - axisHeight, 20, axisHeight);
  
  // calculate bar height
  float dataPoint = 7.63;
  float dataPointHeight = axisHeight * (dataPoint - bottomValue) / (topValue - bottomValue);
  //println(bottomValue);
  //println(dataPoint - bottomValue);
  rect(leftOffset + 20, axisHeight - dataPointHeight, 20, dataPointHeight);
